    ...
    def journal(request):
        ...
        def downloadFile(request):
            ...
            if obj_result is not None:  # если есть данные то
                ...
                for row in obj_result:
                    ...
                    hdr_cells[
                        1
                    ].text = f"{row.user.last_name} {row.user.first_name} {row.user.patronymic}"
                    ...
                    hdr_cells[
                        5
                    ].text = f"{request.user.last_name} {request.user.first_name} {request.user.patronymic}"
                ...
                response["Content-Disposition"] = "attachment; filename=" + dateNameFile
        ...
        if (
            ...
            if request.POST:  # для аякс условия
                if request.POST["id_date_start"] != "":  # если есть дата
                    try:
                        obj_result = res.objects.filter(
                            ...
                            date_instruction__range=(request.POST["id_date_start"], timezone.now()),
                            mark=request.POST["mark"],
                        )
                    except res.DoesNotExist:
                        obj_result = []
                ...
                return JsonResponse(
                    {"result": obj_res, "status": status, "message": message}, safe=False
                )
        else:
                ...
                if request.POST.get("phone_number"):  # если отправили номер телефона
                    if user.phone_number != request.POST["phone_number"]:
                        ...
                        if form.is_valid():
                            ...
                            user.save()
                            data["message"] = (
                                f"{user.first_name}, Вы успешно сменили номер телефона!"
                            )
                            data["status"] = "ok"
                            return JsonResponse(data)
                        else:
                ...
                if request.POST.get("email"):  # если отправили почту
                    if user.email != request.POST["email"]:
                        ...
                        if form.is_valid():
                            ...
                            user.save()
                            data["message"] = (
                                f"{user.first_name}, Вы успешно сменили электронную почту!"
                            )
                            data["status"] = "ok"
                            return JsonResponse(data)
                        else:
...
class BriefBrainView(View):
    ...
    def save(request, num, id):
        ...
        def savetothedb(request):
            ...
            try:
                count_rows = res.objects.get(
                    user=request.user,
                    instruction_id=request.brief_instruction_id,
                    quiz=request.brief_quiz,
                )  # получение строк
                ...
                res.objects.create(
                    ...
                    date_instruction_end=request.brief_days_passed,
                    result=request.brief_score,
                    mark=request.brief_mark,
                    attempt=1,
                )
            ...
            for q in questions:
                ...
                if a_selected != "":  # если ответ не пустой, то
                    ...
                    for a in question_answers:
                        if (
                            a_selected == a.text
                        ):  # если выбранный ответ совпадает с ответов настоящим
                            if a.correct:
    ...
    def checkFile(request, id):
        ...
        if request.method == "POST":
            obj_query = downloadInstructionsForTests.objects.filter(
                user=request.user, test=request.POST["quiz-pk"]
            ).count()
            if obj_query == 0:
                # !!!
                downloadInstructionsForTests.objects.create(
                    user=request.user, test_id=request.POST["quiz-pk"]
                )
                data["status"] = "ok"
            else:
                data["status"] = "warning"
    ...
    def checkPassed(request, id):
        ...
        if request.method == "POST":
            ...
            obj_query_file = downloadInstructionsForTests.objects.filter(
                user=request.user, test=request.POST["quiz"]
            ).count()
            if obj_query_file != 0:
                passed_brief = res.objects.filter(
                    instruction=id, quiz=request.POST["quiz"], mark="Сдан", user=request.user
                ).count()
                if passed_brief == 0:
                    data["status"] = True
                else:
...
def testView(request, num, id):
    ...
    quiz = test.objects.get(instruction=id, type_user=request.user.type_user, pk=num)
    passed_brief = res.objects.filter(
        instruction=id, quiz=quiz, mark="Сдан", user=request.user
    ).count()
...
class BriefLayoutView(View):
    ...
    def listBriefs(request):
        ...
        for i in themesInstructions:
            i.counttest2 = test.get_need_instr(request, i.pk).count() + int(
                briefsForPeoples.objects.filter(
                    test__instruction=i.pk,
                    test__type_user=request.user.type_user,
                    user=request.user,
                ).count()
            )  # вычисление доступных тестов для пользователя
    ...
    def tests(request, id):
        ...
        values = {"test": tests, "themes": themesInstructions, "assigned": assignedBriefs}
