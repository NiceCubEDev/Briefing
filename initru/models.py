...
class Region(models.Model):
    name_region = models.CharField("Название Региона", max_length=64)
...
class Street(models.Model):
    name_street = models.CharField("Название улицы", max_length=64)
...
class City(models.Model):
    ...
    region = models.ForeignKey(Region, on_delete=models.CASCADE, null=False, verbose_name="Регион")
...
class Spec(models.Model):
    name_special = models.CharField("Название специальности", max_length=255)
    ...
    def __str__(self):
        return f"{self.name_special} - {self.encryption_special} "


# роли
class role(models.Model):
    name_role = models.CharField("Название роли", max_length=30)
...
class Groups(models.Model):
    name_group = models.CharField("Название группы", max_length=10)
    special = models.ForeignKey(
        Spec, on_delete=models.CASCADE, null=False, verbose_name="Специальность"
    )
...
class typeuser(models.Model):
    name_type_user = models.CharField("Название типа пользователя", max_length=15)
...
class test(models.Model):
    ...
    required_score_to_pass = models.IntegerField(
        "Рекомендуемый балл в %", help_text="Рекомендуемый балл в %"
    )
    type_user = models.ForeignKey(
        typeuser, on_delete=models.CASCADE, verbose_name="Тип пользователя"
    )
    file = models.FileField("Файл к тесту", upload_to="instr/", null=True, blank=True)
    stud_groups = models.ForeignKey(
        Groups, on_delete=models.CASCADE, verbose_name="Группа", null=True, blank=True
    )
...
class contact_us(models.Model):
    name_contact = models.CharField("Имя пользователя", max_length=125)
    email_contact = models.EmailField(
        "Электронная почта пользователя",
    )
    text_contact = models.TextField("Сообщение пользователя", max_length=350)
...
class complex(models.Model):
    name_complex = models.CharField("Название", max_length=255)
    image_complex = models.ImageField("Изображение", upload_to="images/startPage/complex/")
...
class CustomUser(AbstractUser):  # custom user for Users with django
    ...
    def get_user(request):
        return CustomUser.objects.get(pk=request.user.id)
...
class briefsForPeoples(models.Model):
    user = models.ForeignKey(
        "CustomUser", on_delete=models.CASCADE, verbose_name="Пользователь", related_name="u"
    )
    test = models.ForeignKey(
        "test", on_delete=models.CASCADE, verbose_name="Тест", related_name="t"
    )
    date_target = models.DateTimeField(
        "Время назначения",
    )
